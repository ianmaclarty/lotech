...
LT_DEFS = LT_$(shell echo $(TARGET_PLATFORM) | tr a-z A-Z) LT_$(shell echo $(GRADE) | tr a-z A-Z)
...
ifdef GOOGLE_ADS
  LT_DEFS += LT_GOOGLE_ADS
endif
ifdef STEAMWORKS
  LT_DEFS += LT_STEAMWORKS
endif

LOTECH = $(BUILD_BIN_DIR)/lotech$(EXE_EXT)
...
ifeq ($(TARGET_PLATFORM),html)
  LT_DEPS = $(LUAVM) stb kissfft tinymt
  LOTECH = $(BUILD_BIN_DIR)/lotech.html
else ifdef IOS
  LT_DEPS = $(LUAVM) stb kissfft tinymt
  ifndef NO_METAL
    LT_DEPS += glslopt
    LT_DEFS += LT_USE_METAL
  endif
else ifeq ($(TARGET_PLATFORM),msvc32)
  LT_DEPS = $(LUAVM) stb kissfft tinymt ft2 angle $(STEAMWORKS_DEP)
  ...
  LT_DEPS = $(LUAVM) sdl angle stb kissfft tinymt ft2
  ifndef NO_METAL
    LT_DEPS += glslopt
    LT_DEFS += LT_USE_METAL
  else
...
DEP_ALIBS = $(patsubst %,$(BUILD_LIB_DIR)/lib%$(ALIB_EXT),$(LT_DEPS))

VIEW_TEMPLATES = $(wildcard $(SRC_DIR)/am_view_*.inc)

LT_CPP_FILES = $(sort $(wildcard $(SRC_DIR)/*.cpp))
LT_H_FILES = $(wildcard $(SRC_DIR)/*.h)
LT_OBJ_FILES = $(patsubst $(SRC_DIR)/%.cpp,$(BUILD_OBJ_DIR)/%$(OBJ_EXT),$(LT_CPP_FILES))

LT_INCLUDE_FLAGS = $(INCLUDE_OPT)$(BUILD_INC_DIR) \
	$(INCLUDE_OPT)$(GLM_DIR)

LT_DEF_FLAGS=$(patsubst %,$(DEF_OPT)%,$(LT_DEFS))

LT_CFLAGS = $(LT_DEF_FLAGS) $(LUA_CFLAGS) $(XCFLAGS) $(LT_INCLUDE_FLAGS) $(COMMON_CFLAGS) $(NO_STRICT_ALIAS_OPT)
LT_LDFLAGS = $(GRADE_LDFLAGS) $(DEP_ALIBS) $(XLDFLAGS) $(LDFLAGS)
...
.PHONY: all
ifeq ($(TARGET_PLATFORM),html)
all: $(BUILD_HTML_EDITOR_FILES) $(BUILD_BIN_DIR)/player.html $(BUILD_EXAMPLE_FILES) $(LOTECH)
else
all: $(LOTECH)
endif

ifeq ($(TARGET_PLATFORM),html)
$(LOTECH): $(DEP_ALIBS) $(LT_OBJ_FILES) $(EMSCRIPTEN_LIBS) $(EMSCRIPTEN_PREJS) | $(BUILD_BIN_DIR) 
	$(LINK) $(LT_OBJ_FILES) $(LT_LDFLAGS) $(EXE_OUT_OPT)$@
	@$(PRINT_BUILD_DONE_MSG)
else ifdef IOS
# Also build the static library for iOS for use in Xcode.
$(LOTECH): $(DEP_ALIBS) $(LT_OBJ_FILES) $(EXTRA_PREREQS) | $(BUILD_BIN_DIR)
	rm -f $@$(ALIB_EXT)
	$(AR) $(AR_OPTS) $(AR_OUT_OPT)$@$(ALIB_EXT) $(LT_OBJ_FILES) 
...
$(LOTECH): $(DEP_ALIBS) $(LT_OBJ_FILES) $(EXTRA_PREREQS) | $(BUILD_BIN_DIR)
	rc lotech.rc
	$(LINK) $(CONSOLE_SUBSYSTEM_OPT) lotech.res $(LT_OBJ_FILES) $(LT_LDFLAGS) $(EXE_OUT_OPT)$(BUILD_BIN_DIR)/lotech-console.exe
	$(LINK) $(WINDOWS_SUBSYSTEM_OPT) lotech.res $(LT_OBJ_FILES) $(LT_LDFLAGS) $(EXE_OUT_OPT)$@
	cp $(BUILD_BIN_DIR)/* .
	@$(PRINT_BUILD_DONE_MSG)
else ifdef MINGW
$(LOTECH): $(DEP_ALIBS) $(LT_OBJ_FILES) $(EXTRA_PREREQS) | $(BUILD_BIN_DIR)
	$(LINK) $(CONSOLE_SUBSYSTEM_OPT) $(LT_OBJ_FILES) $(LT_LDFLAGS) $(EXE_OUT_OPT)$(BUILD_BIN_DIR)/lotech-console.exe
	$(LINK) $(WINDOWS_SUBSYSTEM_OPT) $(LT_OBJ_FILES) $(LT_LDFLAGS) $(EXE_OUT_OPT)$@
	cp $(BUILD_BIN_DIR)/* .
	@$(PRINT_BUILD_DONE_MSG)
else ifdef OSX
$(LOTECH): $(DEP_ALIBS) $(LT_OBJ_FILES) $(EXTRA_PREREQS) | $(BUILD_BIN_DIR)
	...
	cp icons/lotech.icns $(BUILD_BIN_DIR)/
	cp $(BUILD_BIN_DIR)/lotech .
	@$(PRINT_BUILD_DONE_MSG)
else
$(LOTECH): $(DEP_ALIBS) $(LT_OBJ_FILES) $(EXTRA_PREREQS) | $(BUILD_BIN_DIR)
	$(LINK) $(LT_OBJ_FILES) $(LT_LDFLAGS) $(EXE_OUT_OPT)$@
...
$(LT_OBJ_FILES): $(BUILD_OBJ_DIR)/%$(OBJ_EXT): $(SRC_DIR)/%.cpp $(LT_H_FILES) $(DEP_ALIBS) $(EXTRA_PREREQS) | $(BUILD_OBJ_DIR) $(EXTRA_PREREQS)
	$(CPP) $(LT_CFLAGS) $(NOLINK_OPT) $< $(OBJ_OUT_OPT)$@
...
gllog: gllog.cpp $(DEP_ALIBS) $(EXTRA_PREREQS) | $(BUILD_BIN_DIR)
	$(CPP) $(LT_CFLAGS) $(NOLINK_OPT) $< $(OBJ_OUT_OPT)gllog.o
	$(LINK) gllog.o $(LT_LDFLAGS) -lGL $(EXE_OUT_OPT)$@
